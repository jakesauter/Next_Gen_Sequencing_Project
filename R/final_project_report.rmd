...
```{r, echo=FALSE}
study_de_genes <- c('LOC102724661', 'ANKRD26P3', 'MOV10L1', 'HIST2H2BA', 'ZBTB8B', 'GLT1D1', 'TNFRSF21', 'CECR2', 'PDD6IPP2', 'PTPRZ1', 'IGSF10', 'GRIA2', 'MEIS1', 'SELENBP1', 'SERPINF1', 'RIMS2', 'ASTN1', 'TLN2', 'ZNF532', 'ZNF662', 'NIN', 'ULK3', 'SLC38A7', 'FOXP1', 'ARSA', 'CTBP1-AS', 'LOC102725328', 'CBX6', 'GYPC', 'FBRSL1', 'SMAD7', 'PLXNC1', 'CLDN15', 'TSHZ3', 'KCNJ5', 'DPYD', 'STEAP3', 'RUNX3', 'PTPRG', 'ACD', 'TTYH3', 'LOC100133445', 'LOC102724549', 'VENTX', 'TM9SF1', 'SMIM3', 'ZNF703', 'TGFBI', 'EMP2', 'PSTPIP1', 'ZNF696', 'RFX2', 'APOE', 'ATOH8', 'ADAM8', 'GAS2L1', 'CHCHD5', 'ADAMTS13', 'A1BG', 'IL15', 'SECTM1', 'FAM109A', 'LOC100507639', 'ZNF843', 'S100A4', 'LSR')

study_de_genes_df <- structure(list(padj = c("0.000041", "0.000041", "0.004718", "0.005319", 
...
```{r, echo=FALSE}
literature_supported_genes_df <- structure(list(padj = c("0.000041", "0.000041", "0.004718", "0.005319", 
"0.019995", "0.019995", "0.019995", "0.019995", "0.037131", "0.041758", 
"0.042672", "0.044855", "0.048952", "0.048952", "0.059128", "0.059128", 
"0.091389", "0.091389", "0.093845", "0.107681", "0.233903", "0.240712", 
"0.240922", "0.240922", "0.272270", "0.302060", "0.351846", "0.361383", 
...
We will now filter the samples previously seen to have low-quality aspects that have the possibility of adding unnecessary noise to our analyses. The samples  `SRR8440524`, `SRR8440538`, and `SRR8440539` will be removed due to cyclic gc-content findings,  `SRR8440463` will be removed due to GC-rich contamination, and finally `SRR8440447`, `SRR8440517`, `SRR8440484`, and`SRR8440488` will be removed due to unknown sex of the sample (a possible confounder).
...
colSums(counts(DESeq.ds)) %>% 
  ...
  ggtitle('Mapped Read Counts Per Sample')
```
...
**P**ost-**M**ortem-**I**nterval, being the time since death that the sample was processed was not found in the original publication to be required to be modeled as a covariate between AD and Control samples, however for peace of mind we also see that PMI is fairly evenly distributed between the two groups.
...
study_selected_gencode_ids <- 
  data.frame(gene_id = literature_supported_genes) %>% 
...
```{r, echo = FALSE}
diff_exp_df %>% 
  filter(gene_name %in% literature_supported_genes, 
         padj < 0.05) %>%
  .$gene_name %>% unname()
```
...
revigo.data <- rbind(c("GO:0017113","dihydropyrimidine dehydrogenase (NADP+) activity",0.002,2.789,0.942,0.000,"dihydropyrimidine dehydrogenase (NADP+) activity"),
c("GO:0052851","ferric-chelate reductase (NADPH) activity",0.001,2.198,0.836,0.159,"dihydropyrimidine dehydrogenase (NADP+) activity"),
c("GO:0038048","dynorphin receptor activity",0.001,2.492,0.983,0.000,"dynorphin receptor activity"),
c("GO:0046911","metal chelating activity",0.000,2.920,0.979,0.000,"metal chelating activity"),
c("GO:0004098","cerebroside-sulfatase activity",0.000,2.773,0.994,0.012,"cerebroside-sulfatase activity"),
c("GO:0002058","uracil binding",0.000,2.466,0.795,0.021,"uracil binding"),
c("GO:0050661","NADP binding",0.673,2.258,0.916,0.135,"uracil binding"),
c("GO:0002054","nucleobase binding",0.001,2.088,0.931,0.144,"uracil binding"),
c("GO:0050682","AF-2 domain binding",0.000,2.120,0.978,0.022,"AF-2 domain binding"),
c("GO:0060228","phosphatidylcholine-sterol O-acyltransferase activator activity",0.001,2.143,0.983,0.095,"phosphatidylcholine-sterol O-acyltransferase activator activity"));
...
In this report, I have confirmed differential expression of **14** of **25** literature supported Alzheimer's Disease associated genes. Below I provide a brief description of each gene based on [NCBI gene information](https://www.ncbi.nlm.nih.gov/gene) and the [GeneCards Human Gene Database](https://www.genecards.org/cgi-bin)
...
|:-------------|:------------------------------|:-------------|
|TrimGalore    |Trimmed Reads                  |[Trimmed Reads](#trimgalore) |
|STAR          |Aligned Samples                |[Aligned Samples](#star) |
|featureCounts |featureCounts                  |[featureCounts](#featurecounts) |
|DESeq2        |Differentially Expressed Genes |[DE Genes](#negative-binomial-wald-test) |
